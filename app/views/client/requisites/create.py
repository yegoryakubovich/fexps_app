...
from app.utils import Fonts, Error, value_to_int
from app.views.client.account.requisite_data.models import RequisiteDataCreateModel
from app.views.client.requests.create import RequisiteDataCreateTypes
...
class RequisiteCreateView(ClientBaseView):
    ...
    async def get_requisite_data_options(self, method_id: int) -> list[Option]:
        ...
        options = [
            Option(text=await self.client.session.gtv(key=f'requisite_create_requisite_data_{key}'), key=key)
